...
"""Install the exact os-haproxy candidate in the running OPNsense CI guest."""
...
COMMIT = os.environ.get("OPNSENSE_HAPROXY_PLUGIN_COMMIT", "")
    ...
    while time.time() < deadline:
        ...
        if status.get("exited"):
            ...
            if status.get("exitcode", 1) != 0:
                ...
                raise RuntimeError(f"os-haproxy installation failed:\n{detail}")
            return stdout
        time.sleep(2)
    raise TimeoutError("os-haproxy installation timed out")
...
make -C /usr/plugins/net/haproxy upgrade
pkg info -e 'os-haproxy-*'
version=$(pkg query '%v' os-haproxy)
test -n "$version"
installed_hash=$(pkg query '%At:%Av' os-haproxy | sed -n 's/^product_hash://p')
test -n "$installed_hash"
case "{commit}" in
    "$installed_hash"*) ;;
    *) echo "installed os-haproxy hash does not match requested commit" >&2; exit 1 ;;
esac
test -f /usr/local/opnsense/mvc/app/models/OPNsense/HAProxy/HAProxy.xml
"""


def main():
    if not COMMIT_RE.fullmatch(COMMIT):
        raise RuntimeError("OPNSENSE_HAPROXY_PLUGIN_COMMIT must be a full Git commit SHA")
    guest_exec(build_install_script(COMMIT))
    print(f"Installed os-haproxy candidate {COMMIT[:12]}")
