...
"""Install the exact os-caddy candidate in the running OPNsense CI guest."""
...
COMMIT = os.environ.get("OPNSENSE_CADDY_PLUGIN_COMMIT", "")
    ...
    while time.time() < deadline:
        ...
        if status.get("exited"):
            ...
            if status.get("exitcode", 1) != 0:
                ...
                raise RuntimeError(f"os-caddy installation failed:\n{detail}")
            return stdout
        time.sleep(2)
    raise TimeoutError("os-caddy installation timed out")
...
make -C /usr/plugins/www/caddy upgrade
pkg info -e 'os-caddy-*'
version=$(pkg query '%v' os-caddy)
test "$version" = '2.2.0'
installed_hash=$(pkg query '%At:%Av' os-caddy | sed -n 's/^product_hash://p')
test -n "$installed_hash"
case "{commit}" in
    "$installed_hash"*) ;;
    *) echo "installed os-caddy hash does not match requested commit" >&2; exit 1 ;;
esac
test -f /usr/local/opnsense/mvc/app/models/OPNsense/Caddy/Caddy.xml
"""


def main():
    if not COMMIT_RE.fullmatch(COMMIT):
        raise RuntimeError("OPNSENSE_CADDY_PLUGIN_COMMIT must be a full Git commit SHA")
    guest_exec(build_install_script(COMMIT))
    print(f"Installed os-caddy candidate {COMMIT[:12]}")
